SUBROUTINE IMF_NORM(mini,mact,nmass,N_IMF,Mret,Metal_eject,element_ej,Nrem,mrem1)
  ...
  REAL(KIND(1.d0)), INTENT(in), DIMENSION(nm)  :: mini,mact
  ...
  REAL(KIND(1.d0)) :: k1,k2
  REAL(KIND(1.d0)) :: M_LOW,M_turn1,M_UP,Mass_isoc,Mrem_isoc
  ...
  k2= 1./( (M_turn1**(alpha1-alpha2))*(M_turn1**(-alpha1+2.)-M_LOW**(-alpha1+2.))/(-alpha1+2.)+&
         &(M_UP**(-alpha2+2.)-M_turn1**(-alpha2+2.))/(-alpha2+2.)  )
...
!-------------------------------
       IF(remnant_cal.EQ.1)THEN
          ...
          IF (z_isoc(zmet).GT. 4.*10.**(-3.)) THEN
            ...
            MCO = -2.0+(B1+2.0)*(0.5/(1.+10.**((KK1-Mrem(j))*&
                   delta1)))+(0.5/(1.+10.**((KK2-Mrem(j))*delta2)))
            ...
            ELSEIF (z_isoc(zmet).GT.5*10.**(-4.)) THEN
             ...
             IF(mrem(j).LE. 2.6) THEN
                  Mret(j)=0.077*mrem(j)+0.48  !mrem(j)/2.   !0.077*mrem(j)+0.48 !
             ENDIF
...
!------------------------------
    ...
    DO j=1,num_rem
        ...
        ENDIF
        IF (mrem(j).GE.M_turn1.AND.mrem(j).LE.M_UP) THEN
