SUBROUTINE csp_gen(tage, mass_csp, mrem_csp, mdwarf_csp, lbol_csp,&
  ...
  ! Internal Variables
  INTEGER :: i, imin, imax, i_young_stars
  ...
  REAL(KIND(1.d0)), DIMENSION(num_time) :: Mgas, Metal_gas, metal
...
!------------------------------------------------
  ...
  DO i=  imin, imax
     ...
     IF (i == imin) THEN
        zmet_new(i) = zmet_ini
     ELSE
        zmet_new(i) = MAX(locate(z_isoc,Metal(i-1)),1)
     ENDIF
...
!----------------------calculate metallicity------------------------------
     ...
     IF (i==imin) THEN
        ...
        Mgas(i) = Mgas_ini - mass_ssp(i) * mass_frac - mrem_ssp(i) * mass_frac
        Metal_gas(i) = Metal_gas_ini - Metal_gas_ini * mass_frac + Metal_ssp(i) * mass_frac
     ELSE
        Mgas(i) = Mgas(i-1) - mass_ssp(i)* mass_frac - mrem_ssp(i) * mass_frac
        Metal_gas(i) = Metal_gas(i-1) - Metal_gas(i-1)*mass_frac + Metal_ssp(i)* mass_frac
     ENDIF

     Metal(i) = Metal_gas(i) / Mgas(i)
